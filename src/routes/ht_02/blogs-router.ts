...
blogsRouter.post(`/`, nameValidation, urlValidation, validationMiddleware, authorization(), (req: Request, res: Response) => {
    const newBlog = blogsRepository.addNewBlog(req.body.name, req.body.youtubeUrl)
    res.status(201).send(newBlog)
})
blogsRouter.put(`/:id`, nameValidation, urlValidation, validationMiddleware, authorization(), (req: Request, res: Response) => {
